...
export default function DesignationsScreen() {
  ...
  const [statusTarget, setStatusTarget] = useState<Designation | null>(null);
  const setStatus = useSetDesignationStatus();

  const primary = useThemeColor({}, 'primary');
  const isSubmitting = createDesignation.isPending;

  const handleSubmit = (values: { name: string; description: string }) => {
    const onSuccess = () => {
      Toast.show({ type: 'success', text1: `Designation ${editing ? 'updated' : 'created'} successfully` });
      setFormOpen(false);
      setEditing(null);
    };
    const onError = (err: unknown) =>
      Toast.show({ type: 'error', text1: 'Unable to save designation', text2: getReadableErrorMessage(err) });

    createDesignation.mutate(values, { onSuccess, onError });
  };
  ...
  const handleToggleStatus = () => {
    ...
    setStatus.mutate(
      ...
      {
        onSuccess: () => {
          Toast.show({ type: 'success', text1: 'Designation deactivated' });
    ...
    <ThemedView style={styles.flex}>
      ...
      {isLoading ? (
        ...
        <FlatList
          ...
          ListEmptyComponent={<EmptyState icon="business-outline" title="No designations found" actionLabel="Add Designation" onAction={() => setFormOpen(true)} />}
        />
      )}
      ...
      <ConfirmDialog
        ...
        description="This designation will no longer be active."
